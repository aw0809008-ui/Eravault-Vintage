...
export function PWARegister() {
  ...
  useEffect(() => {
    ...
    if(/iPad|iPhone|iPod/.test(navigator.userAgent)&&!localStorage.getItem("ev_ios")){setIsIOS(true);setTimeout(()=>setShow(true),3000);}
  ...
  return <div className="fixed bottom-4 left-4 right-4 md:left-auto md:right-4 md:w-72 z-50 animate-fade-in">
    <div className="rounded-xl p-4 bg-surface border border-line shadow-lg">
      <button onClick={()=>{setShow(false);localStorage.setItem("ev_ios","1");}} className="absolute top-3 right-3 text-on-surface-3 cursor-pointer"><X className="w-4 h-4" /></button>
