...
import { Plus, Search, Filter, Edit2, Trash2, ExternalLink, Package, X, ImageIcon, Play, ArrowUpDown, ChevronUp, ChevronDown } from "lucide-react";
...
export default function InventoryPage() {
  ...
  // Only open form once from URL param
  useEffect(() => {
    if (sp.get("add") === "true" && !addHandled.current) {
  ...
  function getAllMedia(item: InventoryItem): string[] {
    return [...(item.images?item.images.split(',').filter(Boolean):[]), ...(item.videos?item.videos.split(',').filter(Boolean):[])];
  }
    ...
    <div className="space-y-5 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
        <div>
          ...
          <p className="text-[13px] text-on-surface-3 mt-0.5">{items.length} item{items.length !== 1 ? 's' : ''} in collection</p>
        </div>
      ...
      {/* Items */}
      {loading ? <div className="space-y-3">{[1,2,3,4].map(i => <Card key={i}><CardContent className="p-4"><div className="flex gap-3"><Skeleton className="w-16 h-16 rounded-xl shrink-0" /><div className="flex-1 space-y-2.5"><Skeleton className="h-4 w-48" /><Skeleton className="h-3 w-32" /><Skeleton className="h-3 w-24" /></div></div></CardContent></Card>)}</div>
      : filtered.length === 0 ? (
        <Card><CardContent className="py-20 text-center">
          ...
          <p className="text-[13px] text-on-surface-3 mt-1.5 max-w-xs mx-auto">{items.length === 0 ? "Add your first vintage piece to start tracking" : "Try different search or filters"}</p>
              ...
              <Card key={item.id} className="group">
                <CardContent className="p-4">
                  <div className="flex items-start gap-3.5">
                    {/* Thumbnail */}
                    {imgs.length > 0 ? (
                      <div className="w-[72px] h-[72px] rounded-xl overflow-hidden border border-line shrink-0 cursor-pointer hover:opacity-80 transition-opacity" onClick={() => setViewMedia(getAllMedia(item))}>
                        <img src={imgs[0]} alt="" className="w-full h-full object-cover" />
                      </div>
                    ) : vids.length > 0 ? (
                      <div className="w-[72px] h-[72px] rounded-xl overflow-hidden border border-line shrink-0 cursor-pointer relative hover:opacity-80 transition-opacity" onClick={() => setViewMedia(getAllMedia(item))}>
                    ...
                    <div className="flex-1 min-w-0">
                      <div className="flex items-start justify-between gap-2">
                        <p className="text-[15px] font-semibold text-on-surface leading-snug pr-2">{item.itemName}</p>
                      ...
                      <div className="flex items-center gap-1.5 mt-1.5 flex-wrap">
                        ...
                        {mediaCount > 0 && <>
                          <span className="text-on-surface-3 text-[10px]">•</span>
                          <span className="text-[11px] text-on-surface-3 flex items-center gap-0.5"><ImageIcon className="w-3 h-3" />{mediaCount}</span>
                        </>}
                      </div>

                      {/* Media row */}
                      {mediaCount > 0 && <MediaThumbnails images={item.images} videos={item.videos} onClick={() => setViewMedia(getAllMedia(item))} />}

                      {/* Price + actions */}
                      <div className="flex items-center justify-between mt-3 pt-3 border-t border-line">
                        <div className="flex items-center gap-3 text-[13px]">
                          <div><span className="text-on-surface-3">Cost </span><span className="font-semibold text-on-surface">{formatCurrency(item.sourcingCost)}</span></div>
                          <div><span className="text-on-surface-3">Price </span><span className="font-bold text-on-surface">{item.sellingPrice ? formatCurrency(item.sellingPrice) : "—"}</span></div>
                          {profit !== null && <span className={`font-bold ${profit >= 0 ? "text-green" : "text-red"}`}>{profit >= 0 ? "+" : ""}{formatCurrency(profit)}</span>}
                        </div>
                        <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                          {item.listingLink && <a href={item.listingLink} target="_blank" rel="noopener noreferrer" className="p-2 rounded-lg hover:bg-surface-2 text-on-surface-3 transition-colors"><ExternalLink className="w-3.5 h-3.5" /></a>}
                          <button onClick={() => openEdit(item)} className="p-2 rounded-lg hover:bg-surface-2 text-on-surface-3 cursor-pointer transition-colors"><Edit2 className="w-3.5 h-3.5" /></button>
                          <button onClick={() => { setDelId(item.id); setDelName(item.itemName); setDelOpen(true); }} className="p-2 rounded-lg hover:bg-red/10 text-red cursor-pointer transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                        </div>
                      </div>
                    </div>
                  </div>
