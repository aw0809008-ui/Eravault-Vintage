...
import { Loader2, Zap, Globe, BarChart3 } from "lucide-react";
    ...
    <div className="min-h-screen flex">
      <div className="hidden lg:flex lg:w-[520px] relative overflow-hidden bg-gradient-to-br from-emerald-600 via-teal-700 to-cyan-800 flex-col justify-between p-12 text-white">
        ...
        <div className="relative z-10">
          <div className="flex items-center gap-3">
            ...
            <div><span className="font-bold text-lg">Eravault</span><span className="text-emerald-200 text-xs block -mt-0.5">VINTAGE</span></div>
          </div>
        </div>

        <div className="relative z-10 space-y-6">
          <h2 className="text-[32px] font-bold leading-[1.2] tracking-tight">Start your vintage<br/>business today</h2>
          <p className="text-emerald-200 text-[15px] leading-relaxed max-w-sm">Join sellers who track their inventory, maximize profits, and scale on Fleek.</p>
          <div className="space-y-3 pt-2">
            {[{icon:Zap,text:"Ready in 30 seconds"},{icon:Globe,text:"Access from any device"},{icon:BarChart3,text:"Real-time analytics"}].map((f,i) => (
              <div key={i} className="flex items-center gap-3">
                <div className="w-8 h-8 rounded-lg bg-white/10 backdrop-blur-sm flex items-center justify-center border border-white/10"><f.icon className="w-4 h-4 text-emerald-200" /></div>
                <span className="text-[14px] text-emerald-100">{f.text}</span>
              </div>
            ))}
          </div>
        </div>
        <p className="relative z-10 text-emerald-300/60 text-xs">Free forever · No credit card required</p>
      </div>
      ...
      <div className="flex-1 flex items-center justify-center px-6 bg-page">
        <div className="w-full max-w-[400px] animate-fade-in">
          ...
          <h1 className="text-[26px] font-bold text-on-surface tracking-tight">Create your account</h1>
          <p className="text-on-surface-3 text-[14px] mt-1.5 mb-8">Free forever · Demo data included</p>

          <form onSubmit={onSubmit} className="space-y-5">
            {error && <div className={`text-[13px] rounded-xl px-4 py-3 flex items-center gap-2 border ${error.includes("email")||error.includes("Check")?"bg-blue-500/10 border-blue-500/20 text-blue-500":"bg-red-500/10 border-red-500/20 text-red-500"}`}><div className={`w-1.5 h-1.5 rounded-full shrink-0 ${error.includes("email")?"bg-blue-500":"bg-red-500"}`} />{error}</div>}
            ...
            <Button type="submit" className="w-full h-11 text-[14px]" disabled={loading}>{loading ? <><Loader2 className="w-4 h-4 animate-spin" />Creating account...</> : "Create account"}</Button>
          </form>

          <div className="mt-8 pt-6 border-t border-line text-center">
            <p className="text-[13px] text-on-surface-3">Already have an account? <Link href="/login" className="text-primary font-semibold hover:underline">Sign in</Link></p>
          </div>
        </div>
      </div>
    </div>
